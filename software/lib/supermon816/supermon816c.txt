...
;	————————————————————————————————————————————————————————————————————————
;
monlsr   bcc .0000010          ;bank entered
;
         lda reg_pbx           ;use current bank...
         bra .0000040          ;as default
;
.0000010 jsr facasize          ;check arg size
         cmp #s_word
         bcc .0000030          ;bank in range
;
.0000020 jmp monerr            ;abort w/error
;
.0000030 lda faca              ;entered bank
;
.0000040 sta loadbank          ;set load bank
         stz pageoff           ;reset page offset
         jsr getparm           ;check for page offset
         bcs .0000050          ;none entered
;
         jsr facasize          ;check size
         cmp #s_word
...
;	acquire record...
;
.0000010 jsr getchanb          ;get datum from channel B
         bcc .0000020          ;gotten
;
         jmp monlsr07          ;aborted by user
;
.0000020 beq monlsr02          ;EOL, process record
;
         sta ibuffer,x         ;store datum
         inx                   ;bump index
         dey                   ;decrement counter
         bne .0000010          ;buffer not full, get next
;
...
;
.0000010 jsr getchar           ;get 1st char from record
...
;
.0000020 jsr getcvt            ;get & convert
...
.0000010 sep #m_setr
         ...
         ldx #0                ;buffer index
;
.0000020 lda ibuffer,x         ;get byte &...
...
;
         ...
         bne .0000010          ;at least 1 record loaded
;
...
;
.0000020 sta reg_pcx           ;set execution address
         sep #m_seta
...
;
.0000010 jsr teststop          ;check for user abort
         bcs .0000040          ;aborted
;
...
;
         ...
         bne .0000010          ;no, wait for next datum
;
.0000020 lda #0                ;end of record
;
.0000030 clc                   ;valid datum
;
.0000040 rts
;
.0000050 wai                   ;wait for any interrupt &...
         bra .0000010          ;check for data
;
;===============================================================================
;
...
getcvt   sep #m_setr
         ...
         beq .0000010          ;premature EOT
;
         jsr nybtobin          ;convert to a nybble
         bcs .0000020          ;conversion error
;
